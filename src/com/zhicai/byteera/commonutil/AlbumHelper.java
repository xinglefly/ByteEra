	...
	private void getAlbumColumnData(Cursor cur) {
		if (cur.moveToFirst()) {
			...
			do {
				...
				// Do something with the values.
				SDLog.i(TAG, _id + " album:" + album + " albumArt:" + albumArt
	...
	void buildImagesBucketList() {
		...
		if (cur.moveToFirst()) {
			...
			do {
				...
				SDLog.d(TAG, _id + ", bucketId: " + bucketId + ", picasaId: "
		...
		while (itr.hasNext()) {
			...
			SDLog.d(TAG, entry.getKey() + ", " + bucket.bucketName + ", " + bucket.count + " ---------- ");
			for (int i = 0; i < bucket.imageList.size(); ++i) {
				ImageItem image = bucket.imageList.get(i);
				SDLog.d(TAG, "----- " + image.imageId + ", " + image.imagePath
						+ ", " + image.thumbnailPath);
			}
		}
		hasBuildImagesBucketList = true;
		long endTime = System.currentTimeMillis();
		SDLog.d(TAG, "use time: " + (endTime - startTime) + " ms");
	}
