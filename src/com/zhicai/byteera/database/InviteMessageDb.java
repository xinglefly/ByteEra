...
import com.zhicai.byteera.database.InviteMessage.InviteMesageStatus;
...
public class InviteMessageDb {

	private ZhiCaiOpenHelper dbHelper;

	public static final String INVITE_TABLE_NAME = "new_friends_msgs";
	public static final String INVITE_COLUMN_NAME_ID = "id";
	...
	public InviteMessageDb(Context context) {
	...
	public void updateMessage(int msgId, ContentValues values) {
		SQLiteDatabase db = dbHelper.getWritableDatabase();
		if (db.isOpen()) {
			db.update(INVITE_TABLE_NAME, values, INVITE_COLUMN_NAME_ID + " = ?", new String[] { String.valueOf(msgId) });
		}
	}
	...
	public List<InviteMessage> getMessagesList() {
		...
		if (db.isOpen()) {
			Cursor cursor = db.rawQuery("select * from " + INVITE_TABLE_NAME + " desc", null);
			while (cursor.moveToNext()) {
				InviteMessage msg = new InviteMessage();
				int id = cursor.getInt(cursor.getColumnIndex(INVITE_COLUMN_NAME_ID));
	...
	public void deleteMessage(String from) {
		SQLiteDatabase db = dbHelper.getWritableDatabase();
		if (db.isOpen()) {
			db.delete(INVITE_TABLE_NAME, COLUMN_NAME_FROM + " = ?", new String[] { from });
		}
	}
}
